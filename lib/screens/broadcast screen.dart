...
import 'package:flutter_twitch_clone_final/config/appid.dart';
import 'package:flutter_twitch_clone_final/resources/firestore_methods.dart';
import 'package:flutter_twitch_clone_final/responsive/responsive_layout.dart';
...
_engine.setClientRole(ClientRole.Broadcaster);
  ...
  String baseUrl = "https://flutter-twitch-final.herokuapp.com";
  String? token;
...
debugPrint('joinChannelSuccess $channel $uid $elapsed');
  ...
  void _joinChannel()async{
    ...
    await _engine.joinChannelWithUserAccount(token, widget.channelId, Provider.of<UserProvider>(context,listen:false).user.uid);
  }
...
await helper.joinChannelWithUserAccount(token,widget.channelId, Provider.of<UserProvider>(context,listen: false).user.uid);
  }
...
return Future.value(true);
                  ...
                  Column(
                    ...
                    children: [
                      ...
                      InkWell(
                        ...
                        child: Text(isScreenSharing? "Stop ScreenSharing":"Start ScreenSharing"),
                      )
                    ],
                  ),
                Expanded(
  ...
  _renderVideo(user,isScreenSharing){
    return AspectRatio(aspectRatio: 16/9,child: "${user.uid}${user.username}" == widget.channelId?isScreenSharing? kIsWeb ?const RtcLocalView.SurfaceView.screenShare():const RtcLocalView.TextureView.screenShare():const RtcLocalView.SurfaceView(
      zOrderMediaOverlay: true,
      zOrderOnTop: true,
    ):isScreenSharing?kIsWeb?const RtcLocalView.SurfaceView.screenShare():const RtcLocalView.TextureView.screenShare():remoteuid.isNotEmpty?kIsWeb?RtcRemoteView.SurfaceView(
      uid: remoteuid[0],
