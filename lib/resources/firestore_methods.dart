...
import 'package:flutter_twitch_clone_final/resources/storage_methods.dart';
      ...
      if(title.isNotEmpty && image != null){
        if(!((await _firestore.collection('livestream').doc("${user.user.uid}${user.user.username}").get())).exists) {
          String thumbnailUrl = await _storageMethods.uploadImageToStorage(
              "livestream_thumbnails", image, user.user.uid);
          channelId = "${user.user.uid}${user.user.username}";
          LiveStream liveStream = LiveStream(title: title,
              image: thumbnailUrl,
              uid: user.user.uid,
          ...
          showSnackBar(context, "Two live Stream cannot Start At same Time");
        }
      }
      else{
   ...
   try{
     ...
     for(int i =0; i<snap.docs.length; i++) {
       await _firestore.collection('livestream').doc(channelId).collection(
           'comments').doc(((snap.docs[i].data()! as dynamic)['commmentId'])).delete();
     }
